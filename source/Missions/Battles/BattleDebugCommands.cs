...
internal static class BattleDebugCommands
{
    ...
    /// Named animation_timeline because coop.debug.battle.timeline is the battle-observation ledger. Armed on demand rather than at battle start: the buffers are small and the interesting seconds are
    /// whenever the player is actually in melee.
    /// </summary>
    public sealed class TimelineCoopCommand : ICoopCommand
    {
        public string Prefix => "coop.debug.battle";

        public string Name => "animation_timeline";

        public string Description => "Records, snapshots or stops the per-tick animation timeline of the battle.";

        public IExpectedArgs[] ExpectedArgs { get; } = new IExpectedArgs[]
        {
            new ExpectedArgs("action", "start, snapshot, stop or status."),
        };
        ...
        public CoopCommandResult ProcessCommand(ICoopCommandArgs args)
        {
            switch (args[0].ToLowerInvariant())
            {
                case "start":
                    ...
                    return Succeeded("Animation timeline recording STARTED.");
                case "snapshot":
                    return Succeeded("BATTLE_TIMELINE " + AnimationTimeline.Snapshot(stop: false));
                case "stop":
                    return Succeeded("BATTLE_TIMELINE " + AnimationTimeline.Snapshot(stop: true));
                case "status":
                    return Succeeded("Animation timeline is "
                        + (AnimationTimeline.Enabled ? "RECORDING." : "idle."));
                default:
                    return Failed("Usage: coop.debug.battle.animation_timeline <start|snapshot|stop|status>");
            }
        }
    }
    ...
    /// </summary>
    public sealed class WindupCoopCommand : ICoopCommand
    {
        public string Prefix => "coop.debug.battle";

        public string Name => "windup";

        public string Description => "Starts, snapshots or stops the remote attack wind-up and delivery measurement.";
        ...
        public CoopCommandResult ProcessCommand(ICoopCommandArgs args)
        {
            switch (args[0].ToLowerInvariant())
            {
                case "start":
                    ...
                    return Succeeded("Remote attack wind-up and delivery measurement is ON.");
                case "snapshot":
                    return Succeeded("BATTLE_WINDUP " + WindupDiagnostics.Snapshot(stop: false)
                ...
                case "stop":
                    return Succeeded("BATTLE_WINDUP " + WindupDiagnostics.Snapshot(stop: true)
                ...
                case "status":
                    return Succeeded("Remote attack wind-up measurement is "
                        + (WindupDiagnostics.Enabled ? "ON." : "OFF."));
                default:
                    return Failed("Usage: coop.debug.battle.windup <start|snapshot|stop|status>");
            }
        }
    }
#endif
    ...
    /// </remarks>
    public sealed class EngineSizeCoopCommand : ICoopCommand
    {
        public string Prefix => "coop.debug.battle";

        public string Name => "engine_size";

        public string Description => "Reports the engine battle size against the coop battle size, and whether they agree.";
        ...
        public CoopCommandResult ProcessCommand(ICoopCommandArgs args)
        {
            ...
            catch (Exception e) { return Failed($"ENGINE_BATTLE_SIZE error=could not read the engine size: {e.GetType().Name}"); }
            ...
            return Succeeded($"ENGINE_BATTLE_SIZE sliderIndex={index} engineSize={engineSize} " +
