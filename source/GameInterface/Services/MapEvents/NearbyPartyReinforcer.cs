...
internal interface INearbyPartyReinforcer
{
    ...
    void ReinforceOpenPlayerBattlesIgnoringCampaignClock();
...
internal sealed class NearbyPartyReinforcer : INearbyPartyReinforcer
{
    ...
    /// <summary>Re-scans every open player battle without consulting the campaign clock.</summary>
    /// <remarks>
    /// The scheduled sweep above is driven by campaign time, and campaign time STOPS for the duration of a
    /// co-op battle - PlayerOccupancyPauseHandler pauses as soon as every player is occupied. The schedule
    /// therefore can never come due while a battle is actually being fought, so the battle-start scan would
    /// be the only one that ever runs and a lord who was slightly too far away at the opening bell could
    /// never join. That is the case this exists for.
    ///
    /// Driven from LiveBattleReinforcementTickPatch on a real-time cadence instead. The per-battle schedule
    /// is deliberately left untouched, so once the campaign resumes the ordinary sweep carries on unchanged.
    /// </remarks>
    public void ReinforceOpenPlayerBattlesIgnoringCampaignClock()
    {
        var encounterModel = Campaign.Current?.Models?.EncounterModel;
        if (encounterModel == null) return;
