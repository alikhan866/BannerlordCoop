...
public class MapEventDebugCommands
{
    ...
    public sealed class StartNearestBanditAttackCoopCommand : ICoopCommand
    {
        ...
        public IExpectedArgs[] ExpectedArgs { get; } = new IExpectedArgs[]
        {
            ...
            new ExpectedArgs("max_troops", "Troops kept in the player's party (default 8). A bare number in the second slot is read as this.", false),
        };
        ...
        public CoopCommandResult ProcessCommand(ICoopCommandArgs args)
        {
            ...
            {
                return Failed($"maxTroops must be a whole number, got '{troopsArgument}'.");
            }
            if (maximumFixtureTroops < 1)
            {
                return Failed("maxTroops must be at least 1 - a player with no troops cannot field a battle.");
            }
    ...
    public sealed class ConversationHoldStateCoopCommand : ICoopCommand
    {
        ...
        public IExpectedArgs[] ExpectedArgs { get; } = new IExpectedArgs[]
        {
            new ExpectedArgs("party_id", "A MobileParty id, PartyBase id or party StringId; resolved to the PartyBase id the tracker keys on.", true),
        };
        ...
        public CoopCommandResult ProcessCommand(ICoopCommandArgs args)
        {
            ...
            if (tracker?.ObjectManager == null) return Failed("Conversation tracker is unavailable.");
            ...
            if (!resolved)
                return Failed($"UNKNOWN ID '{args[0]}' - not registered on this instance. This is NOT the same as 'released'.");

            if (!tracker.TryGetEngagement(resolvedId, out var engagement))
                return Succeeded($"Conversation hold for {resolvedId}: released (no engagement registered).");

            return Succeeded($"Conversation hold for {resolvedId}: HELD by engagerParty={engagement.EngagerPartyId} " +
    ...
    // coop.debug.map_event.add_player_to_battle <hostControllerId> <joinerControllerId>
    /// <summary>
    ...
    /// </para>
    /// </remarks>
    public sealed class AddPlayerToBattleCoopCommand : ICoopCommand
    {
        public string Prefix => "coop.debug.map_event";

        public string Name => "add_player_to_battle";

        public string Description => "Puts one player's party onto another player's side of a battle already in progress.";

        public IExpectedArgs[] ExpectedArgs { get; } = new IExpectedArgs[]
        {
            new ExpectedArgs("host_controller_id", "The controller id of the player already in the battle."),
            new ExpectedArgs("joiner_controller_id", "The controller id of the player to add to the host's side."),
        };
        ...
        public CoopCommandResult ProcessCommand(ICoopCommandArgs args)
        {
            ...
            if (args[0] == args[1])
                return Failed("The host and the joiner must be different players.");
            ...
            if (hostSide == null)
                return Failed($"Host {args[0]} is not in a battle.");

            var mapEvent = hostSide.MapEvent;
            if (mapEvent == null)
                return Failed("The host's battle side has no map event.");

            if (mapEvent.IsFinalized)
                return Failed("The host's battle is already finalized.");

            var existingSide = joinerParty.Party.MapEventSide;
            if (existingSide == hostSide)
                return Failed($"{args[1]} is already fighting on that side.");
            if (existingSide != null)
                return Failed($"{args[1]} is already in a different battle - leave it first.");
            ...
            if (joined == null)
                return Failed("The joining party was not added to the battle side.");
            ...
            if (!ContainerProvider.TryResolve<INetwork>(out var network)
                ...
                || !objectManager.TryGetId(mapEvent, out string mapEventId))
            {
                return Failed("Joined the side, but the encounter ids could not be resolved to notify the client.");
            }
            ...
            return Succeeded($"Added {args[1]} ({joinerPartyId}) to {args[0]}'s battle {mapEventId}; " +
