...
coop battle LOSS.";
    ...
    /// </remarks>
    /// <summary>Kills an agent, crediting the blow to the local player when there is one.</summary>
    /// <remarks>
    /// For callers outside this file that have no attacker of their own to name. It routes through the
    /// two-argument overload rather than rebuilding the blow, so the attribution fix above applies to
    /// them as well instead of quietly reverting to a self-inflicted kill.
    /// </remarks>
    internal static void Kill(Agent agent) => Kill(agent, Agent.Main);
