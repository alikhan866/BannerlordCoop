        ...
        bool bypassFastForwardPolicies = false)
    {
        lock (timeControlLock)
        {
                ...
                TryGetDisallowingPolicy(unpausePolicies, out var unpausePolicy))
            {
                ...
                // C30 - published here rather than at each handler that wants a pause, because THIS is where
                // a pause is actually imposed. Instrumenting the handlers instead missed the dominant case:
                // the campaign was held paused by a policy REFUSING to unpause, which no handler ever
                // announces. One site, and it names whichever policy is responsible - including later ones.
            ...
            // Nothing limited the request, so any earlier reason is stale. A reason that outlives its cause
            // would explain away a real stall.
