...
namespace GameInterface.Services.GameDebug.Commands
{
            ...
            public sealed class ClanDailyGoldChangeCoopCommand : ICoopCommand
    {
        ...
        /// </remarks>
        public sealed class WatchClanCoopCommand : ICoopCommand
        {
            public string Prefix => "coop.debug.clan";

            public string Name => "watch";

            public string Description => "Starts an hourly [ClanWatch] report of what a clan's lords are doing.";

            public IExpectedArgs[] ExpectedArgs { get; } = new IExpectedArgs[]
            {
                new ExpectedArgs("clan", "The clan's name or id. Quote multi-word names."),
            };
            ...
            public CoopCommandResult ProcessCommand(ICoopCommandArgs args)
            {
                ...
                if (name.Length == 0)
                    return Failed("Usage: coop.debug.clan.watch <clan name or id>");
        ...
        public sealed class WatchClanOffCoopCommand : ICoopCommand
        {
            public string Prefix => "coop.debug.clan";

            public string Name => "watch_off";

            public string Description => "Stops the clan-activity watch for one clan, or for every clan when no name is given.";

            public IExpectedArgs[] ExpectedArgs { get; } = new IExpectedArgs[]
            {
                new ExpectedArgs("clan", "The clan's name or id. Quote multi-word names. Omit to stop every watch.", false),
            };

            public CoopCommandResult ProcessCommand(ICoopCommandArgs args)
            {
                var name = args.ElementAtOrDefault(0)?.Trim();
                if (name != null && name.Length == 0) name = null;
                var removed = Clans.Handlers.ClanActivityWatchHandler.Unwatch(name);

                if (name == null) return Succeeded(removed ? "Stopped watching every clan." : "No clans were being watched.");

                return Succeeded(removed ? $"Stopped watching '{name}'." : $"'{name}' was not being watched.");
            }
        }

        // coop.debug.clan.watch_list
        public sealed class WatchClanListCoopCommand : ICoopCommand
        {
            public string Prefix => "coop.debug.clan";

            public string Name => "watch_list";

            public string Description => "Lists the clans under the clan-activity watch.";
            ...
            public CoopCommandResult ProcessCommand(ICoopCommandArgs args)
            {
                ...
                return Succeeded(watching.Count == 0
        ...
        public sealed class WatchClanNowCoopCommand : ICoopCommand
        {
            public string Prefix => "coop.debug.clan";

            public string Name => "watch_now";

            public string Description => "Writes a [ClanWatch] report for every watched clan right now.";
            ...
            {
                var watching = Clans.Handlers.ClanActivityWatchHandler.Watching;
                if (watching.Count == 0) return Failed("No clans are being watched; nothing to report.");

                Clans.Handlers.ClanActivityWatchHandler.ReportAll();

                return Succeeded("Wrote a [ClanWatch] report for: " + string.Join(", ", watching));
            }
        }
    }
}
