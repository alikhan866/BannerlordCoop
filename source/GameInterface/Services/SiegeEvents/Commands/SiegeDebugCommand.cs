...
public class SiegeDebugCommand
{
    ...
    /// </remarks>
    public sealed class SallyStateCoopCommand : ICoopCommand
    {
        public string Prefix => "coop.debug.siege";

        public string Name => "sally_state";

        public string Description => "Prints every input to vanilla's sally-out decision for a besieged settlement.";

        public IExpectedArgs[] ExpectedArgs { get; } = new IExpectedArgs[]
        {
            new ExpectedArgs("settlementId", "The besieged settlement's registered id or StringId."),
        };

        public CoopCommandResult ProcessCommand(ICoopCommandArgs args)
        {
            if (ModInformation.IsClient) return Failed("Run this command on the server.");
            if (!TryResolveSettlement(args[0], out var settlement)) return Failed($"Settlement '{args[0]}' not found");

            var sb = new StringBuilder();
            sb.AppendLine($"--- sally-out decision for {settlement.Name} ({settlement.StringId})");

            AppendSortiePreconditions(sb, settlement);
            if (settlement.SiegeEvent == null) return Succeeded(sb.ToString());
