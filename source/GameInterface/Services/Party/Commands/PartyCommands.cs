...
internal class PartyCommands
{
    ...
    /// </summary>
    /// <remarks>
    /// The member-roster twin cannot do this: it looks the row up in MainPartyTroops, and a prisoner is
    ...
    /// </remarks>
    public sealed class StagePartyScreenPrisonerReleaseCoopCommand : ICoopCommand
    {
        public string Prefix => "coop.debug.mobile_party";

        public string Name => "stage_party_screen_prisoner_release";

        public string Description => "Stages a prisoner release on the open Party screen, as clicking the prisoner row does.";

        public IExpectedArgs[] ExpectedArgs { get; } = new IExpectedArgs[]
        {
            new ExpectedArgs("character_id", "The prisoner's character id."),
            new ExpectedArgs("count", "How many to release (default 1).", false),
        };
        ...
        public CoopCommandResult ProcessCommand(ICoopCommandArgs args)
        {
            ...
            if (countArgument != null && (!int.TryParse(countArgument, out count) || count < 1))
                return Failed("Count must be a positive whole number.");
            ...
            if (row == null) return Failed($"{args[0]} is not in the right prisoner roster.");
            ...
            return Succeeded($"PARTY_SCREEN_PRISONER_RELEASE_STAGED character={args[0]} requested={count} " +
    ...
    /// </remarks>
    public sealed class RemoveTroopsCoopCommand : ICoopCommand
    {
        public string Prefix => "coop.debug.mobile_party";

        public string Name => "removetroops";

        public string Description => "Removes a number of one troop type from a party's member roster.";

        public IExpectedArgs[] ExpectedArgs { get; } = new IExpectedArgs[]
        {
            new ExpectedArgs("party_id", "The registered mobile party id."),
            new ExpectedArgs("character_id", "The troop's character id."),
            new ExpectedArgs("count", "How many to remove; clamped to what is present."),
        };
        ...
        public CoopCommandResult ProcessCommand(ICoopCommandArgs args)
        {
            ...
            if (TryGetObjectManager(out var objectManager) == false) return Failed("Unable to resolve ObjectManager.");
            if (!objectManager.TryGetObject(args[0], out MobileParty party)) return Failed($"Party with id {args[0]} not found");
            if (!objectManager.TryGetObject(args[1], out CharacterObject troop)) return Failed($"Troop with id {args[1]} not found");
            if (!int.TryParse(args[2], out var requested) || requested <= 0) return Failed($"'{args[2]}' is not a positive troop count");
            ...
            if (troop.IsHero)
                return Failed($"{troop.Name} is a hero. Removing a hero from a roster detaches them from their party " +
                       "entirely; use the hero-aware repair instead of removetroops.");

            var roster = party.MemberRoster;
            int present = roster.GetTroopCount(troop);
            if (present <= 0) return Failed($"{party.Name} has no {troop.Name} to remove.");
            ...
            return Succeeded($"Removed {toRemove}x {troop.Name} from {party.Name} ({party.StringId}); " +
    ...
    /// </summary>
    public sealed class SetTroopsCoopCommand : ICoopCommand
    {
        public string Prefix => "coop.debug.mobile_party";

        public string Name => "set_troops";

        public string Description => "Replaces a party's rank and file with an exact number of one troop type, keeping heroes.";

        public IExpectedArgs[] ExpectedArgs { get; } = new IExpectedArgs[]
        {
            new ExpectedArgs("party_id", "The registered mobile party id."),
            new ExpectedArgs("troop_id", "The troop's character id, e.g. battania_fian_champion."),
            new ExpectedArgs("count", "The exact number of that troop to end up with."),
        };
        ...
        public CoopCommandResult ProcessCommand(ICoopCommandArgs args)
        {
            ...
            if (!objectManager.TryGetObject(args[0], out MobileParty party)) return Failed($"Party with id {args[0]} not found");
            if (!objectManager.TryGetObject(args[1], out CharacterObject troop)) return Failed($"Troop with id {args[1]} not found");
            if (!int.TryParse(args[2], out var count) || count < 0) return Failed($"'{args[2]}' is not a troop count");
            ...
            return Succeeded($"{party.Name} ({party.StringId}) now has {count}x {troop.Name} plus its heroes; {party.MemberRoster.TotalManCount} total");
        }
    }
    ...
    /// </remarks>
    public sealed class AddHeroMemberCoopCommand : ICoopCommand
    {
        public string Prefix => "coop.debug.mobile_party";

        public string Name => "add_hero_member";

        public string Description => "Puts a hero into a party's member roster, so defeating that party captures them.";

        public IExpectedArgs[] ExpectedArgs { get; } = new IExpectedArgs[]
        {
            new ExpectedArgs("party_id", "The registered mobile party id."),
            new ExpectedArgs("hero_id", "The hero's registered id."),
        };
        ...
        public CoopCommandResult ProcessCommand(ICoopCommandArgs args)
        {
            ...
            if (TryGetObjectManager(out var objectManager) == false) return Failed("Unable to resolve ObjectManager.");
            if (!objectManager.TryGetObject(args[0], out MobileParty party)) return Failed($"Party with id {args[0]} not found");
            if (!objectManager.TryGetObject(args[1], out Hero hero)) return Failed($"Hero with id {args[1]} not found");
            if (!hero.IsAlive) return Failed($"Hero {hero.StringId} is not alive.");
            if (hero.IsPrisoner) return Failed($"Hero {hero.StringId} is a prisoner; free them first.");
            ...
            if (hero.PartyBelongedTo == party)
                return Succeeded($"ADD_HERO_MEMBER hero={hero.StringId} party={party.StringId} inParty=true alreadyThere=true " +
                       $"partyMembers={party.MemberRoster.TotalManCount}");

            party.MemberRoster.AddToCounts(hero.CharacterObject, 1);

            return Succeeded($"ADD_HERO_MEMBER hero={hero.StringId} party={party.StringId} " +
    ...
    /// </remarks>
    public sealed class SetPrisonersCoopCommand : ICoopCommand
    {
        public string Prefix => "coop.debug.mobile_party";

        public string Name => "set_prisoners";

        public string Description => "Sets a party's prisoner roster to an exact number of one troop type.";

        public IExpectedArgs[] ExpectedArgs { get; } = new IExpectedArgs[]
        {
            new ExpectedArgs("party_id", "The registered mobile party id."),
            new ExpectedArgs("troop_id", "The troop's character id."),
            new ExpectedArgs("count", "The exact number of that troop to hold prisoner."),
        };
        ...
        public CoopCommandResult ProcessCommand(ICoopCommandArgs args)
        {
            ...
            if (!objectManager.TryGetObject(args[0], out MobileParty party)) return Failed($"Party with id {args[0]} not found");
            if (!objectManager.TryGetObject(args[1], out CharacterObject troop)) return Failed($"Troop with id {args[1]} not found");
            if (!int.TryParse(args[2], out var count) || count < 0) return Failed($"'{args[2]}' is not a prisoner count");
            ...
            return Succeeded($"SET_PRISONERS party={party.StringId} troop={args[1]} count={count} " +
