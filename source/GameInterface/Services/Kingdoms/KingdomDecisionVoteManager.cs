    ...
    public class KingdomDecisionVoteManager : IKingdomDecisionVoteManager, IDisposable
    {
        ...
        public void ResolveDecisionsNoLongerWaitingOnAnyone()
        {
            ...
            foreach (var decision in DecisionStates.Keys.ToList())
            {
                ...
                // Connected clans only, and deliberately NOT the round snapshot that
                // TryResolveDecision consults. A round holds whoever was eligible when it opened, so a
                // player who has since gone still owes it a vote and the decision cannot reach
                // HasAllVotes - everyone else sits on the Done screen until the round deadline expires.
                // This path is what makes that immediate instead: once nobody still connected owes a
                // vote, the decision resolves now. The deadline remains as the backstop for a player who
                // is connected but simply never answers.
                state.RefreshEligibleClanIds(GetVotingClanIds(decision));
                if (state.EligibleClanIds.Count == 0) continue;
                if (!state.HasAllVotes) continue;
        ...
        /// <summary>Every player clan in this kingdom that a decision may be put to.</summary>
        /// <remarks>
        /// This answers "is this a decision players vote on, or one the AI election settles", and
        /// <c>KingdomInterface.AddDecision</c> asks it on the server AND on every client to pick that branch.
        /// Clients hold no peers for anyone, so an unguarded connection test here would empty the set on every
        /// client and have them settle as an AI election a decision the server was putting to a vote. The
        /// <c>IsServer</c> guard below is what prevents that: clients keep every player clan, and only the
        /// server drops the ones whose player is gone.
        ///
        /// Note the two are therefore NOT identical while someone is disconnected. That is deliberate and
        /// safe, because the branch turns only on whether the set is EMPTY, and it can only empty on the
        /// server once every player clan in the kingdom has dropped - at which point no client is watching it.
        ///
        /// Whether a vote is still OWED is a separate question, asked against the round snapshot by
        /// <see cref="RefreshEligibleClanIds"/> and against live connections by
        /// <see cref="GetVotingClanIds"/>, both on the server where the resolution barrier lives.
        /// </remarks>
